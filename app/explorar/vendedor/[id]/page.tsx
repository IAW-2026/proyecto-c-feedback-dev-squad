...
import { verifyTokenAny } from '../../../../lib/handoffToken'
...
export default async function VendedorReviewsPage({ params, searchParams }: Props) {
  ...
  if (!clerkUserId && sp?.token) {
    const verified = await verifyTokenAny(
      [
        { secret: process.env.BUYER_APP_SELLER_KEY!, origin: 'buyer' },
        { secret: process.env.API_KEY_SELLER_APP!, origin: 'seller' },
      ],
      sp.token as string,
      id
    )
          ...
          {tokenOrigin ? (
            ...
            <div className="flex flex-row flex-wrap items-center justify-between gap-2">
              ...
              >
                ...
                <span className="sm:hidden text-xs">Volver</span><span className="hidden sm:inline">Volver a explorar</span>
              </Link>
              <ThemeLink
                href="https://proyecto-c-seller-dev-squad.vercel.app/dashboard/reviews"
                aria-label="Ir a la app de vendedores"
              ...
              >
                ...
                <span className="sm:hidden text-xs">Ir a ventas</span><span className="hidden sm:inline">Ir a la app de vendedores</span>
              </ThemeLink>
            </div>
          )}
        </div>
