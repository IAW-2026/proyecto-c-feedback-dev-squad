...
export default function ExplorarPage() {
  ...
  const [data, setData] = useState<PaginatedResponse<SearchResult>>({ data: [], total: 0, page: 1, limit: 10, totalPages: 0 })
  ...
  const selectTarget = (item: SearchResult) => {
    router.push(item._tipo === 'product' ? `/explorar/producto/${item.id}` : `/explorar/vendedor/${item.id}`)
  }
              ...
              {data.data.map(item => (
                <li key={`${item._tipo}:${item.id}`}>
                  <button
                    ...
                    className="w-full text-left bg-white dark:bg-gray-800 rounded-2xl border border-gray-300 dark:border-gray-700 p-4 shadow hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                  >
