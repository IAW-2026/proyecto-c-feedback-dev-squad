...
export default function MisResenasClient() {
  ...
  const [tipoFilter, setTipoFilter] = useState<ReviewType | 'all'>(
    () => (searchParams.get('tipo') as ReviewType | 'all') || 'all',
  )

  const [data, setData] = useState<PaginatedResponse<Review>>({
    data: [],
    total: 0,
    page: 1,
    limit: 6,
  ...
  const syncUrl = (p: number, s: string, tipo: ReviewType | 'all') => {
  ...
  const updateParams = (updates: Record<string, string | undefined>) => {
    ...
    const newTipo = 'tipo' in updates ? ((updates.tipo as ReviewType | 'all') || 'all') : tipoFilter
    setPage(newPage)
    setSearch(newSearch)
    setTipoFilter(newTipo)
    syncUrl(newPage, newSearch, newTipo)
  }
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-lg text-gray-500 dark:text-gray-400">
              {search || tipoFilter !== 'all' ? 'No se encontraron resultados' : 'Todavía no publicaste reseñas'}
            </p>
          </div>
        ) : (
          <>
